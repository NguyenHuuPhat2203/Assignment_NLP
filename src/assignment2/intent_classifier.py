...
def _filter_labeled(
    ...
    filtered_c, filtered_l = (
        ...
        if any(l != "Unknown" for l in labels)
        else ([], [])
    )
...
def main(output_dir: str | None = None, clauses_file: str | None = None) -> None:
    ...
    try:
        ...
        if len(labeled_clauses) >= 8:
            bert_clf.train(
                (labeled_clauses, labeled_labels), save_path="models/intent_bert"
            )
        else:
            logger.warning(
        ...
        logger.warning(
            "BERT training/inference failed (%s); falling back to TF-IDF labels.", exc
        )
    ...
    logger.info(
        "Intent classification written to %s (%d clauses).", intent_output, len(clauses)
    )
