...
_CHROMA_DIR = str(
    _PROJECT_ROOT / os.getenv("CHROMA_PERSIST_DIR", "./chroma_db").lstrip("./")
)
...
with st.sidebar:
    ...
    if collection_size > 0:
        ...
        st.warning(" Index is empty. Click **Build Index** to index clauses.")
...
def _render_sources(sources: list[dict], show_meta: bool) -> None:
    ...
    with st.expander(
        f" Sources ({len(sources)} clause{'s' if len(sources) != 1 else ''})"
    ):
...
st.markdown(
    "Ask any question about the legal contract. Answers are grounded in the contract text."
)
    ...
    with st.chat_message("assistant"):
        if st.session_state.vector_store.get_collection_size() == 0:
            ...
            with st.spinner("Thinking…"):
                try:
                    result = st.session_state.pipeline.chat(st.session_state.messages)
